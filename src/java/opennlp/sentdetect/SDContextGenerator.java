 ...
 * @version     $Revision: 1.1 $, $Date: 2003/03/07 03:50:09 $
 */
...
public class SDContextGenerator implements ContextGenerator {
     ...
     */    
    private static final int previousSpaceIndex(StringBuffer sb,
                                                int seek) {
        seek--;
        while (seek > 0) {
            if (sb.charAt(seek) == ' ') {
     ...
     * @return The index which contains the nearest space.
     */    
    private static final int nextSpaceIndex(StringBuffer sb,
                                            int seek,
                                            int lastIndex) {
        seek++;
        char c;
